...
<CodeGroup>
  ...
  const response = await fetch('https://ContextGPT.co/api/v1/chatbots', {
    headers: {
      'Authorization': 'Bearer YOUR_API_KEY'
    }
  });
  ...
  response = requests.get(
      'https://ContextGPT.co/api/v1/chatbots',
...
<CodeGroup>
  ```bash cURL theme={null}
  curl -X POST https://ContextGPT.co/api/v1/chatbots \
    -H "Authorization: Bearer YOUR_API_KEY" \
  ...
  ```php PHP theme={null}
  <?php
  $ch = curl_init('https://ContextGPT.co/api/v1/chatbots');
...
* **Email** - [support@ContextGPT.co](mailto:support@ContextGPT.co)
